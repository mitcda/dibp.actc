...
### Created:      2017-09-28
### Updated:      <2017-10-05 21:07:38 david at grover>
### Author:       David Mitchell <david.p.mitchell@homemail.com.au>
### Description:  Build Australian Customs Tariff Classification
###               \url{https://www.border.gov.au/Busi/cargo-support-trade-and-goods/importing-goods/tariff-classification-of-goods/current-tariff-classification}
...
Data.url <- "https://www.border.gov.au/Busi/cargo-support-trade-and-goods/importing-goods/tariff-classification-of-goods/current-tariff-classification/schedule-3";

####  Return Schedule 3 - Section numbers, names and Hrefs
Sections <- Data.url %>%
  html_session %>%
  read_html %>%
  html_nodes("a") %>%
...
raw.Chapters <- Sections$section_href %>%
  Map(function(x) Data.url %>% url_parse %>%
                  inset("path", value=x) %>%
                  url_compose %>%
                  html_session %>%
                  read_html %>%
...
Chapters <- Map(function(x, i) data.frame(section_href = i,
                                          href = html_attr(x, "href"),
                                          text = html_text(x)),
                raw.Chapters, names(raw.Chapters)) %>%
  bind_rows %>%
  ...
  filter(!grepl("^Chapters\\W*\\d+", text)) %>%             ## Remove text == 'Chapters xx'
  spread(type, text) %>%
...
if (DEBUG) {
  Chapters %>% 
    write.csv(file=file.path(tempdir(), "Chapters.csv"), row.names=FALSE)
}
...
actc <- Tables %>%
  ...
  left_join(Chapters, by="chapter_href") %>%
  left_join(Sections, by="section_href") %>%
...
devtools::use_data(actc_2017, overwrite=TRUE);
